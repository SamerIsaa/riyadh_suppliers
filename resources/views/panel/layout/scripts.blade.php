...
<script>
    ...
    $(document).on('click', '.deletes', function () {
        ...
        checkedItems = datatable.getSelectedRecords();
        $.each(checkedItems, function () {
            ids.push($(this).find('input').val());
        });
        // checkedNames.splice(0, 1);
        if (checkedItems.length != 0) {
            Swal.fire({
                title: '@lang('constants.confirm_process')',
                icon: 'warning',
                showCancelButton: true,
                cancelButtonText: '@lang('constants.cancel')',
                confirmButtonText: '@lang('constants.confirm')'
            }).then(function (result) {
                if (result.value) {
                    $.ajaxSetup({
                        headers: {
            ...
            swal.fire({
                title: 'الرجاء اختيار حقل على الاقل للحذف ',
                type: 'error',
